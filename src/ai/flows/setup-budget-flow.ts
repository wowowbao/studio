...
'use server';
/**
 * @fileOverview An AI agent to set up a budget from an image or PDF.
 *
 * - setupBudgetFromImage - A function that handles budget setup using an image or PDF.
...
const SetupBudgetInputSchema = z.object({
  imageDataUri: z
    .string()
    .describe(
      "An image or PDF of a budget document (e.g., handwritten notes, spreadsheet screenshot, financial plan PDF), as a data URI that must include a MIME type and use Base64 encoding. Expected format: 'data:<mimetype>;base64,<encoded_data>'."
    ),
});
...
const SuggestedSubCategorySchema = z.object({
  ...
  budgetedAmount: z.number().optional().describe("The suggested budgeted amount for this subcategory. If not clearly identifiable from the document, leave this field blank."),
});
...
Analyze the provided image or document, which could be a handwritten budget, a photo of a spreadsheet, a PDF of a financial plan, or similar financial planning document.
...
For each category:
1.  Identify its name.
2.  If the category has subcategories listed in the document:
...
4.  If a budgeted amount for any category or subcategory is not clearly identifiable from the document, leave its 'budgetedAmount' field blank.
5.  Do not invent categories or amounts not present or clearly implied by the document.
6.  If the document does not appear to be a budget or financial plan, or if no structure can be determined, return an empty list of categories or set an 'aiError'.

Budget Document (Image or PDF):
...
const setupBudgetFlow = ai.defineFlow(
  ...
  async (input: SetupBudgetInput) => {
    if (!input.imageDataUri.startsWith('data:')) { // Generic check for data URI
        return { aiError: 'Invalid file data URI format. Please provide a valid image or PDF.' };
    }
    ...
    try {
      ...
      if (!output || !output.categories || output.categories.length === 0) {
        ...
        return { aiError: 'AI could not determine a budget structure from the document. Please try a clearer file or a different document.' };
      }
      return output;
    } catch (e: any) {
